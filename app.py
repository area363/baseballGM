...
def player_pick():
    ...
    if db.myteam.count_documents({}) > 7:
    ...
    if db.myteam.count_documents({ 'Name':name_receive }, limit = 1) != 0:
        return jsonify({'result': 'fail'})
    else:
        db.myteam.insert_one(player)

    count = db.myteam.count_documents({})
...
def player_delete():
    ...
    if db.myteam.count_documents({ 'Name':name_receive }, limit = 1) == 0:
        ...
        db.myteam.remove({'Name':name_receive})
        count = db.myteam.count_documents({})
        return jsonify({'result': 'success', 'count': count})
...
@app.route('/api/myteamlist', methods=['GET'])
def myplayer_list():
    # 1. mystar 목록 전체를 검색합니다. ID는 제외하고 like 가 많은 순으로 정렬합니다.
    # 참고) find({},{'_id':False}), sort()를 활용하면 굿!
    # 2. 성공하면 success 메시지와 함께 stars_list 목록을 클라이언트에 전달합니다.

    players = list(db.myteam.find({}, {"_id" : 0}).sort('rank',-1))
    return jsonify({'result': 'success','msg':'list 연결되었습니다!', 'data' : players})
